...
public class UserController {
	private RDFConnectionRemoteBuilder builder = RDFConnectionFuseki.create().destination("http://localhost:3030/user");

	@RequestMapping("/new")
	public Boolean insert(@RequestBody UserData reco) {
...
//		System.out.println("password    "+password);
	...
	public Boolean delete(@RequestBody UserData reco) {
	...
	public Boolean update(@RequestBody UserData reco) {
		...
		try (RDFConnectionFuseki conn = (RDFConnectionFuseki) builder.build()) {
			...
			request.add("PREFIX dc: <http://www.w3.org/2001/vcard-rdf/3.0#>\r\n" + "PREFIX ab:<http://userdata/#>\r\n"
					...
					+ userName + "\".\r\n" + "}");
			request.add("PREFIX dc: <http://www.w3.org/2001/vcard-rdf/3.0#>\r\n" + "PREFIX ab:<http://userdata/#>\r\n" +
					"INSERT DATA{ab:" + name + " dc:name \"" + name + "\"; dc:username \"" + userName
					+ "\" ; dc:password \"" + password + "\" .}");
			conn.update(request);
		}
		return true;
	...
	public String select2(@RequestBody UserData reco) {
		String name = reco.getName();
		// String userName = reco.getUsername();
		// In this variation, a connection is built each time.
